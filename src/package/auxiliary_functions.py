...
def align_matrices_by_columns(A, B):
    ...
    # Note that the matrices must be more tall than wide.
...
def get_L21_errors(true, estimated):
    factor_match = match_factors(true, estimated)
    estimated = align_matrices_by_columns(true, estimated[:, factor_match])
...
def get_cos_errors(true_data: list, estimated_data: list) -> np.array:
    ...
    #return np.sum(true_data * estimated_data, axis=0)/(np.linalg.norm(true_data, axis = 0) * np.linalg.norm(estimated_data, axis = 0) )
...
def match_factors(true, estimated):
    if np.shape(true)[1] > 1:
        corr_matrix = np.corrcoef(estimated.T, true.T)[:estimated.shape[1], estimated.shape[1]:].T
        factor_match = np.argmax(np.abs(corr_matrix), axis=1)
    return factor_match

def get_idividual_cos_error(true, estimated):
    factor_match = match_factors(true, estimated)
    new_estimated = align_matrices_by_columns(true, estimated[:,factor_match])
    errors = np.zeros(new_estimated.shape[1])
    for factor in range(new_estimated.shape[1]):
        errors[factor] = get_cos_errors(true[:,factor].reshape(-1,1), new_estimated[:, factor].reshape(-1,1))
    return errors

    
def get_spectral_norm(true, estimated):
    if np.shape(true)[1] > 1:
...
def old_alternating_least_squares(target_Y, factors, convergence_criteria):
    """
        ...
        # Also big oof this code in general. Readability and generalisabilty = 0
    """
...
def alternating_least_squares(target_Y, factors, convergence_criteria, max_iters=1000):
    ...
    return updated_factors, counter

def unfold(tensor, mode):
    shape = np.shape(tensor)
    return np.moveaxis(tensor,mode,0) \
            .reshape((shape[mode],-1),order='F')

def tpca(tensor, factors):
    order = np.ndim(tensor)
    print(f"order {order}")
    s_hat = np.empty(shape = order, dtype = object)
    m_hat = np.zeros(shape = order, dtype = object)
    print(s_hat)
    for mode in range(order):
        #print(f"This is the mode {mode}")
        s, gamma = LA.eigh(unfold(tensor, mode) @ unfold(tensor, mode).transpose()) # Eigen-decomposition
        #print(f"Ths is shat {np.sqrt(np.sort(s)[::-1][:factors]).real}")
        s_hat[mode] = np.sqrt(np.sort(s)[::-1][:factors]).real # scale components
        m_hat[mode] = gamma[:,s.argsort()[::-1][:factors]].real # vector components
...
def fit_var(data):
        ...
        # Hacky fix is to mak  e it be at least 1 always
        # Hardcoded as one for the moment.
        fitted_model = model.fit(max(lag_order.aic,1)) 
        return fitted_model, lag_order.aic

def predict_var(data, steps):
    # So for some reason sometimes on the rotated data we get that the VAR is just a constant
    # Hacky fix is to make it be at least 1 always
...
def simulate_3d(range_zip, repetitions, noise_scale, dgp, **kwargs):
    row_dict = {f"factor_{i}_err": "" for i in range(len(range_zip))}
    #print(f"This is row_dict {row_dict}")
    #print(f"list(range_zip) {range_zip}")
    row_dict["sample_size"] = ""
    row_dict["repetition"] = ""
    rows_list = []
    print(f"Theses are the passd kwargs {kwargs}")
    dgp = dgp(**kwargs)
    #print(f"Range zip in the fuction call {range_zip}")
    for sample_size in range_zip:
        print(f"Going for sample size {sample_size}")
        for rep in range(repetitions):
            seed = rep

            M, s = dgp.generate_data(sample_size)

            Y = dgp.set_Y(noise_scale) # We have modified the shape in the generate data call
            print(dgp.lags)
            print("dgp lags")
            Z = tensorpca.TensorPCA(Y)
            s_hat, M_hat = Z.t_pca((dgp.lags+1)*dgp.n_factors)
            M_hat_ALS = Z.ALS(convergence_criteria = 0.0001, max_iters = 1000)

            for i in range(len(M_hat_ALS)):
                row_dict[f"factor_{i}_err"] = np.mean(aux.get_idividual_cos_error(M[i], M_hat_ALS[i]))
            row_dict["sample_size"] = sample_size
            row_dict["repetition"] = rep
            rows_list.append(row_dict.copy())

    print(f"Done")
    print(f"Thi is the final row list \n{rows_list}")
